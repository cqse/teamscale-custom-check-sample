 ...
 * inner classes). The declarations are persisted in a teamscale-internal database for later access
 * in a custom check.
 * 
 ...
 * </pre>
 * 
 * The information can be accessed "by uniform path" (returns all declaration objects for a given
 * uniform path) or "by value" (returns all declaration objects for e.g., "inner"). For the second
 * access pattern, {@link #needsAccessByValue()} must be overwritten and must return true.
 */
public class SamplePhase
		implements IGlobalExtractionPhase<SamplePhase.ClassDeclarationInfo, SamplePhase.CustomLocation> {
	...
	public boolean needsAccessByValue() {
		/*
		 * The checks accessing the results of this phase need to access "by value" (e.g.,
	...
	public List<ClassDeclarationInfo> extract(ITokenElementContext context) throws CheckException {
		...
		for (ShallowEntity entity : ShallowEntityTraversalUtils.listEntitiesOfType(entities, EShallowEntityType.TYPE)) {
			...
			CustomLocation location = new CustomLocation(context.getUniformPath());
	...
	@Override
	public ClassDeclarationInfo createValue(String uniformPath, String value, CustomLocation additionalInformation) {
		return new ClassDeclarationInfo(value, additionalInformation);
	}

	/**
	 * "Transport object" that stores the name and location of a class declaration (might declare an
	 * inner class).
	 * 
	 * The result of a check phase are a list of such {@link IExtractedValue} objects which are stored
	 * associated with the currently analyzed file.
	 */
	public static class ClassDeclarationInfo implements IExtractedValue<CustomLocation> {
		...
		public ClassDeclarationInfo(String className, CustomLocation location) {
			this.className = className;
			this.location = location;
		}

		@Override
		public CustomLocation getAdditionalInformation() {
	...
	/**
	 * Location resolved by this phase.
	 * 
	 * @param uniformPath
	 */
	// This should be a custom class, to verify the (de)serialization (TS-41465)
	public record CustomLocation(String uniformPath) implements Serializable {
